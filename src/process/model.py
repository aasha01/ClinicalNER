...
from src.notes.documents import labels as tag2id, id2tag

from src.process.tools import flatten, save_list_structure, reconstruct_list
from src.process.tools import print_str, print_vec, print_files, write

import src.process.DatasetCliner_experimental as Exp

import tensorflow as tf
from src.process import entity_lstm as entity_model
from src.process import training_predict_LSTM
import pickle
import copy
from src.process import helper_dataset as hd
import shutil
from src.Xtemplate_machine_learning import crf
...
class ClinerModel:
    ...
    def train(self, train_notes, val=[], test=[]):
        ...
        if val:
            ...
            self.train_fit(train_sents, train_labels, val_sents=val_sents, val_labels=val_labels, test_sents=test_sents,
    ...
    def predict_classes(self, tokenized_sents):
        ...
        if self._use_lstm:
            if self.parameters == None:
                hyperparams['parameters'] = hd.load_parameters_from_file("LSTM_parameters.txt")
...
def generic_train(p_or_n, train_sents, train_labels, use_lstm, val_sents=None, val_labels=None, test_sents=None,
    ...
    if use_lstm:
        ...
        while epoch_number < 90:
            ...
            if epoch_number != 0:
                ...
                for sequence_number in sequence_numbers:
                    loss, accuracy, transition_params_trained = training_predict_LSTM.train_step(sess, dataset,
                ...
                model_saver.save(sess,
                                 os.path.join(parameters['model_folder'], 'model_{0:05d}.ckpt'.format(epoch_number)))
            ...
            if epoch_number > 0:
                ""
                f1, predictions = training_predict_LSTM.prediction_step(sess, dataset, "test", model, epoch_number,
                                                                        parameters['conll_like_result_folder'],
                                                                        transition_params_trained)
                f1_train, _ = training_predict_LSTM.prediction_step(sess, dataset, "train", model, epoch_number,
                                                                    parameters['conll_like_result_folder'],
                                                                    transition_params_trained)
                f1_valid, _ = training_predict_LSTM.prediction_step(sess, dataset, "valid", model, epoch_number,
                                                                    parameters['conll_like_result_folder'],
                                                                    transition_params_trained)

                correctly_predicted_tokens = training_predict_LSTM.compute_train_accuracy(
                    parameters['conll_like_result_folder'] + "valid" + os.sep + "epoche_" + str(epoch_number) + ".txt")
        ...
        from src.Xtemplate_feature_extraction.features import extract_features
...
def generic_predict(p_or_n, tokenized_sents, vocab, clf, use_lstm, hyperparams):
    ...
    if use_lstm:
        ...
        token_to_vector = dataset.load_dataset(Datasets_tokens, Datasets_labels, "", parameters,
                                               token_to_vector=tokens_to_vec, pretrained_dataset=pretrained_dataset)
        ...
        with sess.as_default():
            ...
            # model=entity_model.EntityLSTM(dataset,parameters)
            transition_params_trained = model.restore_from_pretrained_model(parameters, dataset, sess,
                                                                            token_to_vector=token_to_vector,
                                                                            pretrained_dataset=pretrained_dataset)
            del token_to_vector
            predictions = training_predict_LSTM.prediction_step(sess, dataset, "deploy", model, 0,
                                                                parameters['conll_like_result_folder'],
                                                                transition_params_trained)
            sess.close()
